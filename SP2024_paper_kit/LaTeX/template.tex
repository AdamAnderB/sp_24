...
\title{Paper Template for Speech Prosody 2024}
%Paper submission must be anonymous. Only fill in author information for the final PDF.
\name{Author Anonymous$^1$, Co-author Anonymous$^2$}
...
\begin{abstract}
  ...
  Note that paper submission will be anonymous and that author information should only be added to the final version of the PDF. In the anonymous paper submissions, the author information lines in the template should be left unchanged to reserve space for this information in the final PDF.
...
\begin{table}[t]
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    Title                    & Title                                       \\
    Author                   & Author name (only in final PDF)              \\
    Affiliation              & Author affiliation (only in final PDF)       \\
    Email                    & Email address (only in final PDF)           \\
...
The INTERSPEECH organizing committee does not offer the possibility to submit multimedia files. Instead, multimedia files should be uploaded in open-access platforms such as OSF and authors should provide a link in the text only. These files are meant for audio-visual illustrations that cannot be conveyed in text, tables and graphs. Just like you would when including graphics, make sure that you have sufficient author rights to the multimedia materials that you link in your paper. 

Users of the proceedings who wish to access your multimedia files will click the link to the multimedia files which the author has uploaded in an open-access platform, which will then be opened by the operating system of their computer. Access to the contents of the linked files will be governed entirely by the operating system of the user's computer.
...
The total length of the abstract is limited to 200 words. The abstract included in your paper and the one you enter during web-based submission must be highly similar, with only minor revisions for the final paper. Avoid non-ASCII characters or symbols as they may not display correctly in the abstract book. 

\subsection{Author affiliation}

Please list country names as part of the affiliation for each country. Author affiliations are only included in the final PDF, paper submission will be anonymous.
...
Authors are requested to submit PDF files of their manuscripts. You can use commercially available tools or for in-stance http://www.pdfforge.org/products/pdfcreator. The PDF file should comply with the following requirements: (a) there must be no PASSWORD protection on the PDF file at all; (b) all fonts must be embedded; (c) the file must be text searchable (do CTRL-F and try to find a common word such as ???the???); and (d) the paper submission file must be anonymous, meaning author information should be removed from the file???s metadata. The proceedings editors will contact authors of non-complying files to obtain a replace??ment. In order not to endanger the preparation of the pro??ceedings, papers for which a replacement is not provided in a timely manner will be withdrawn.
...
The Speech Prosody 2024 organizing committee would like to thank the organizing committees of the past INTERSPEECH and Speech Prosody conferences for kindly providing the template files. \\
