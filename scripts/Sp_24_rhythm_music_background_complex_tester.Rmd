...
centered_all_data <- all_data %>%
 # mutate(across(c(language_dprime:melody_score), ~ . - mean(. , na.rm = TRUE)))%>%
  #mutate(across(c(language_dprime:melody_score), ~ (. - min(., na.rm = TRUE)) / (max(., na.rm = TRUE) - min(., na.rm = TRUE))))%>%
  select(Participant.Private.ID:melody_score)%>%
  select(c(Participant.Private.ID:gs_emotions),MET_dprime_Melody,MET_dprime_Rhythm,c(mean_working_memory:melody_score))
  pivot_longer(cols=gs_activeeng:melody_score,names_to = "task",values_to = "scores")%>%
  mutate(rhythm=if_else(grepl("hythm|beat", task),1,0),
         melody=if_else(grepl("elody", task),1,0),
         perception=if_else(grepl("MET", task),1,0),
         production=if_else(grepl("_score", task),1,0),
         gold_smiths=if_else(grepl("gs_", task),1,0),
         working_memory=if_else(grepl("working", task),1,0),
         skill=if_else(melody==1|rhythm==1,1,0),
         music=if_else(gold_smiths==1|gold_smiths==1,1,0))%>%
  mutate(across(c(rhythm:music), as.factor))
...
m_data_m1 <- lm(language_dprime ~scores+production+rhythm+
                  scores:perception+
                  scores:production+
                  scores:rhythm+
                  scores:melody,
...
m_data_m2 <- lmerTest::lmer(language_dprime ~
                  melody+rhythm+gold_smiths+perception+
                  (scores:melody)+
                  (scores:rhythm)+
                  (scores:gold_smiths)+
                  (scores:perception)+
...
m_data_m3 <- lmerTest::lmer(language_dprime ~
                  melody+rhythm+gold_smiths+perception+
                  (scores:melody)+
                  (scores:rhythm)+
                  (scores:gold_smiths)+
...
m_data_m4 <- lmerTest::lmer(language_dprime ~
                  melody+rhythm+perception+
                  (scores:melody)+
                  (scores:rhythm)+
                  (scores:gold_smiths)+
                  (1|task),
...
m_data_m5 <- lmerTest::lmer(language_dprime ~
                  melody+rhythm+perception+
                  (scores:melody)+
                  (scores:rhythm)+
                  (1|task),
...
```{r}
#prep for models
summary(centered_all_data)
centered_all_data <- all_data %>%
  mutate(across(c(language_dprime:melody_score), ~ . - mean(. , na.rm = TRUE)))%>%
  mutate(across(c(language_dprime:melody_score), ~ (. - min(., na.rm = TRUE)) / (max(., na.rm = TRUE) - min(., na.rm = TRUE))))%>%
  select(Participant.Private.ID:melody_score)%>%
  select(c(Participant.Private.ID:language_dprime),mean_working_memory,c(gs_activeeng:gs_emotions),MET_dprime_Melody,MET_dprime_Rhythm,beat_score,melody_score)%>%
  pivot_longer(cols=c(gs_activeeng:gs_emotions),names_to = "gs_tasks",values_to = "gs_scores")%>%
  pivot_longer(cols=c(MET_dprime_Melody,melody_score),names_to = "melody_tasks",values_to = "melody_scores")%>%
  pivot_longer(cols=c(beat_score,MET_dprime_Rhythm),names_to = "beat_tasks",values_to = "beat_scores")
...
m_data_m1 <- lm(language_dprime ~
                              mean_working_memory+
                              gs_tasks*gs_scores+
                              melody_tasks*melody_scores*beat_tasks*beat_scores,
        data=m_data)
summary(m_data_m1)
...
i_data_m5 <- lm(language_dprime ~
                              mean_working_memory+
                              gs_tasks+gs_scores+
                              beat_tasks+beat_scores,
        data=i_data)
summary(i_data_m5)

i_data_m6 <- lm(language_dprime ~
                              mean_working_memory+
                              gs_tasks+gs_scores+
                              beat_scores,
        data=i_data)
...
anova(i_data_m5,i_data_m6)
anova(i_data_m6,i_data_m7)
i_model<-i_data_m2
...
anova(m_data_m4,m_data_m5)
anova(m_data_m5,m_data_m6)
#model three is the parsimonisous model due to better no co-linearity. Simplest takes into account most variance
m_model<-m_data_m6
...
p2
save_ggplot(p2,width = 4.5,height=4.5)
```
