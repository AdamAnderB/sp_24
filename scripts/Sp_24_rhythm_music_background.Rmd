...
#reliability test
cronbach_alpha <- cronbach.alpha(gs_data_agg%>%select(gs_activeeng:gs_general))
...
mad_standard=3
before_mad_removal<-nrow(met_data)
...
#data kept
removed_met_percent<-after_mad_removal/before_mad_removal
removed_met_percent
...
mij_data<-mij_data%>%filter(Participant.Private.ID %in% keep$Participant.Private.ID)
```
...
p1<-longer_all_data%>%
  ...
  scale_color_manual(values =c("black", "red"))+
  scale_y_discrete(limits=rev)+
  labs(title = title, x = "normalized centered scores", y = "density", fill = "\n",alpha="") +
  theme_minimal()+
  theme(legend.position = c(.9, .8),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.x = element_blank(),
