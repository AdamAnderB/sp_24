...
save_ggplot(p1,width = 3,height=3)
...
geom_density_ridges(color="white",alpha=.5)+
  ...
  facet_grid(.~l2_coded,
             labeller = labeller(l2_coded = 
                                   c("l2 match" = "L2 match","non l2 match" = "L2 no match"))) 
...
p1<-all_models %>%
  ...
  geom_errorbar(aes(ymin = conf_lower, ymax = conf_higher), position = dodge_width, width = 0.1,linewidth=1.5) +
  scale_x_discrete(limits=rev)+
  coord_flip() + 
  theme_minimal() +
  labs(
...
save_ggplot(p2,width = 5.5,height=7)
```
...
dynamic_models<-all_models%>%
  ...
  select(!part)%>%
  mutate(Value = ifelse(grepl("p.value", Statistic) & Value < 0.001, 0.001, Value)) %>%
  mutate(Value =as.numeric(Value))%>%
  mutate(Value = case_when(grepl("p.value", Statistic) & Value <= 0.001~ 0.001,
                           grepl("p.value", Statistic) & Value <= 0.01 & Value > 0.001 ~ 0.01,
                           grepl("p.value", Statistic) & Value <= 0.05 & Value > 0.01 ~ 0.05,
                           grepl("p.value", Statistic) & Value > 0.05 ~100,
                           TRUE ~ Value))%>%
  mutate(Value = format(round(Value, digits = 2), nsmall = 2))%>%
  mutate(Value = ifelse(grepl("p.value", Statistic) & Value < 0.0, 0.001, Value))
