...
#data pull
data_list <- readRDS(file.path(master_path,"rds_folder","all_experiments_combined_list_cleaned.rds"))
```
...
#participant info no longer appicable because of the non-cmu students don't have this:
part_info<-data_list$participant_info
part_info$Participant.Private.ID<-as.character(part_info$Participant.Private.ID)
View(part_info)
```
...
p1<-data_rhythm_complete%>%
  ...
  geom_vline(data = vlines,aes(xintercept = xintercepts),color = "#D3D3D3",alpha=.5)+
  scale_x_continuous(limits = c(0, 2450),breaks = c(400,800,1200,1600,2000))+
  scale_fill_manual(values =c("#FDB515","#008F91"))+
  scale_y_discrete(limits=rev)+
  labs(title = title, x = "Time course in MS", y = "Beat number within trial", fill = "Beat or No Beat\n",alpha="") +
  theme_minimal()+
...
all_data<-mij_d_prime%>%
  ...
  mutate(across(gs_activeeng:gs_general, ~ . / 100*2),
         L2s=paste(lang_dominance_order_2, lang_dominance_order_3,
                   lang_dominance_order_4, lang_dominance_order_5, sep = ", "),
         L1=lang_dominance_order_1)%>%
  select(Participant.Private.ID,Spreadsheet.Name,L2s,everything())%>%
  mutate(l2_coded = if_else(str_detect(tolower(L2s), fixed(tolower(Spreadsheet.Name))), "l2 match", "non l2 match"))%>%
  mutate(l1_coded = if_else(str_detect(tolower(lang_acquisition_order_1), fixed(tolower(Spreadsheet.Name))), "l1 match", "non l1 match"))%>%
  mutate(music_coded = as.factor(if_else(str_detect(most_played_instrument, "None|none|N/A"), "non music", "music")))%>%
  select(Participant.Private.ID,Spreadsheet.Name,L2s,L1,l1_coded,l2_coded,music_coded,experiment,most_played_instrument,education,everything())%>%
  select(-language_c)
...
title<-"Normalized score distrubutions by task connections"
p1<-longer_all_data%>%
  ggplot(aes(x=updated_tasks,y=values,color=music_coded))+
  geom_point(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name,color=music_coded)),alpha=1,size=.14)+
  geom_line(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name,color=music_coded)),alpha=.2,size=.15)+
  geom_boxplot(aes(fill=music_coded),color="white",alpha=.4,width=.5)+
  theme_minimal()+
...
title<-"Normalized score distrubutions by task"
p1<-longer_all_data%>%
  ggplot(aes(y=updated_tasks,x=values,group= interaction(as.factor(music_coded),as.factor(task)),fill = as.factor(music_coded),show.legend = F))+
  geom_density_ridges(color="white",alpha=.5)+
  scale_x_continuous(limits = c(-2, 2),breaks = c(-2,-1,0,1,2))+
  scale_fill_manual(values =c("black", "red"))+
  scale_color_manual(values =c("black", "red"))+
  scale_y_discrete(limits=rev)+
  labs(x = "normalized centered scores", y = "density", fill = "\n",alpha="") +
  theme_minimal()+
  theme(legend.position = c(.5, .1),
        ...
        panel.grid.major.y = element_blank())+
  facet_grid(.~l2_coded)
save_ggplot(p1,width = 5.5,height=5.5)

key<-longer_all_data%>%select(Participant.Private.ID,l2_coded,l1_coded,music_coded)
mij_d_prime_keyed<-mij_d_prime%>%
  right_join(key)

title<-"dprime density across tasks"
p1<-mij_d_prime_keyed%>%
  ggplot(aes(y=Spreadsheet.Name,x=language_dprime,group= interaction(as.factor(music_coded),as.factor(Spreadsheet.Name)),fill = as.factor(music_coded),show.legend = F))+
geom_density_ridges(color="white",alpha=.5)+
  ...
  scale_y_discrete(limits=rev)+
  labs(title = title, x = "Dprime", y = "density", fill = "\n",alpha="") +
  theme_minimal()+
  theme(legend.position = c(.9, .8),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.x = element_blank(),
...
tester<-longer_all_data%>%
  ...
  mutate(classroom_coded = case_when(Spreadsheet.Name=="Mandarin" & Language == "Mandarin Chinese"~"match",
                                     Spreadsheet.Name=="italian" & Language == "Italian"~"match",
                                     Spreadsheet.Name=="japanese" & Language == "japanese"~"match",
                                     Spreadsheet.Name=="Mandarin" & Language != "Mandarin Chinese"~"no match",
                                     Spreadsheet.Name=="italian" & Language != "Italian"~"no match",
                                     Spreadsheet.Name=="japanese" & Language != "japanese"~"no match",))
...
combined_plot <- plot_grid(
  plot_grid(empty_plot, density_plot, ncol = 2, rel_widths = c(3.5, 12)),  # Adjust 1 and 4 for the spacer and plot width
  p1,
  ncol = 1,
...
combined_plot
save_ggplot(combined_plot,width=7,height=4)


title="by classroom"
p1<-tester%>%
  ggplot(aes(x=updated_tasks,y=values,color=ratings))+
  #geom_point(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name,color=ratings)),
         #    alpha=1,size=.14)+
  #geom_boxplot(aes(color=ratings),fill="white",alpha=1,width=1)+
  geom_line(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name,color=ratings)),
            alpha=.5,size=.3)+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  facet_grid(.~classroom_coded)+
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
        panel.grid.minor.x = element_blank(),
...
p1<-longer_all_data%>%
    ggplot(aes(x=updated_tasks,y=values,color=music_coded))+
  geom_point(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name,color=music_coded)),
             alpha=1,size=.14)+
  geom_line(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name,color=music_coded)),
            alpha=.2,size=.15)+
  geom_boxplot(aes(fill=music_coded),color="white",alpha=.4,width=.5)+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
...
title<-"dtasks connection students2"
p1<-longer_all_data%>%
  ggplot(aes(x=updated_tasks,y=values,color=music_coded,alpha=.2))+
  geom_point(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name,color=music_coded)),
            alpha=1,size=.14)+
  geom_line(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name,color=music_coded)),
            alpha=.2,size=.5)+
  ...
  scale_color_manual(values = c("black", "red"))+
  scale_fill_manual(values = c("black", "red"))+
  facet_wrap(l2_coded~Spreadsheet.Name)+
  scale_color_manual(values = c("black", "red"))+
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
        panel.grid.minor.x = element_blank())+
...
title<-"dtasks connection students3"
p1<-longer_all_data%>%
  ggplot(aes(x=updated_tasks,y=values,color=music_coded))+
  geom_line(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name)),alpha=1,size=.08)+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  facet_grid(.~l2_coded)+
  scale_color_manual(values = c("black", "red"))+
...
title<-"dtasks connection students4"
p1<-longer_all_data%>%
  ggplot(aes(x=updated_tasks,y=values,color=music_coded))+
  geom_line(aes(group=interaction(Participant.Private.ID,Spreadsheet.Name)),alpha=1,size=.08)+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  facet_grid(Spreadsheet.Name~l2_coded)+
  scale_color_manual(values = c("black", "red"))+
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
        panel.grid.minor.x = element_blank())+
...
#model three is the parsimonisous model
m_model<-m_data_m3
...
anova(j_data_m4,j_data_m5)
#model 4 is the parsimonious model
j_model<-j_data_m4
```
```{r}
check_model(m_model)
check_model(i_data_m5)
check_model(j_model)
```
```{r}
#full model
...
names<-all_models%>%
  select(term)%>%
  unique()
names$updated_terms<-c("Intercepts","L1 (non match)","L2 (non match)", "Music student (non match)",
                       "Goldsmiths general","Goldsmiths engagement","Goldsmiths perception",
                       "Goldsmiths training","Goldsmiths singing","Goldsmiths emotions",
                       "Musical Ear Test d' Melody","Musical Ear Test d' Rhythm","Working-memory",
                       "Auditory-motor Rhythm","Auditory-motor Melody")
...
p1<-all_models %>%
  ...
  geom_point(position = dodge_width,shape=19) +
  geom_text(aes(label=significance,y=-2),position=dodge_width,width = 0.1,size=5,show_guide=F)+
  geom_errorbar(aes(ymin = conf_lower, ymax = conf_higher), position = dodge_width, width = 0.1) +
  scale_x_discrete(limits=rev)+
  coord_flip() + 
  theme_minimal() +
  labs(
...
save_ggplot(p2,width = 5.5,height=5.5)
```
...
#participant removal
starting_participants<-nrow(data_list$Digit_Span__Advanced)
...
keys <- c("starting_participants", "kept_participants", "removed_participants")
values <- c(starting_participants, kept_participants, removed_participants)
